...
num_heroes = 10
counter_range = timedelta(days=13)
increment_delta = timedelta(days=9)


def generate_graph(player, auto_open=True):
	print(f"generating: {player}")
	...
	for hero in heroes:
		...
		data.append(go.Scatter(
			...
			line = { 
				"color": hero["hero"].color
			}
		))


	py.plot(data, filename=f"hero_data_{player}.html", auto_open=auto_open)


generate_graph(player_id)